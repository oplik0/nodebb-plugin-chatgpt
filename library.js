...
plugin.init = async (params) => {
	...
	const { setting1, setting2 } = await meta.settings.get('chatgpt');
	if (setting1) {
	 ...
	 * */
	routeHelpers.setupPageRoute(router, '/chatgpt', [(req, res, next) => {
		winston.info(`[plugins/chatgpt] In middleware. This argument can be either a single middleware or an array of middlewares`);
		setImmediate(next);
	}], (req, res) => {
		winston.info(`[plugins/chatgpt] Navigated to ${nconf.get('relative_path')}/chatgpt`);
		res.render('chatgpt', { uid: req.uid });
	});

	routeHelpers.setupAdminPageRoute(router, '/admin/plugins/chatgpt', [], controllers.renderAdminPage);
};
 ...
 *   curl -X GET \
 * 		http://example.org/api/v3/plugins/chatgpt/test \
...
plugin.addRoutes = async ({ router, middleware, helpers }) => {
	...
	routeHelpers.setupApiRoute(router, 'get', '/chatgpt/:param1', middlewares, (req, res) => {
